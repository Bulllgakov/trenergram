...
**Mini Apps (папка `webapp/`, только через Telegram):**
...
- **Клиенту:** зависит от времени создания тренировки
  - **Если тренировка создана ДО времени первого напоминания:**
    - Уведомление не приходит сразу
    - Первое напоминание придет в запланированное время (например, завтра в 20:00)
  - **Если тренировка создана ПОСЛЕ времени первого напоминания:**
    - Уведомление приходит НЕМЕДЛЕННО (при создании тренировки)
    - Текст: "Придешь на тренировку {дата} в {время}?"
    - Кнопки: [✅ Подтверждаю] [❌ Не смогу прийти]
    - Далее работает цепочка напоминаний (второе, третье, автоотмена)
...
- **При подтверждении клиентом:**
  - Клиент может подтвердить 2 способами:
    ...
    2. В боте (кнопка "✅ Подтверждаю" в напоминании или немедленном уведомлении)
...
- **Если тренировка создана после времени первого напоминания** - клиент получает немедленное уведомление
...
- **Цепочка напоминаний работает всегда:** второе → третье → автоотмена (независимо от того, было ли первое напоминание запланированным или немедленным)

#### Немедленные уведомления при позднем создании тренировки (Статус: ✅ РЕАЛИЗОВАНО 2025-11-24)

**Проблема:**
Когда тренер создает тренировку на завтра после времени первого напоминания (например, в 14:00, а напоминание настроено на 12:00), клиент не получал никакого уведомления, так как время первого напоминания уже прошло.

**Решение:**
При создании тренировки система проверяет, не прошло ли время первого напоминания. Если прошло - отправляет немедленное уведомление клиенту.

**Алгоритм работы:**
```
1. Тренер создает тренировку (booking.created_by = "trainer")
2. Система вызывает notify_booking_created_by_trainer()
3. Вычисляется время первого напоминания:
   - reminder_datetime = booking_date - reminder_1_days_before at reminder_1_time
   - Пример: тренировка 25.11 в 18:00, настройка "за 1 день в 12:00"
   - reminder_datetime = 24.11 в 12:00
4. Сравнение с текущим временем (в timezone тренера)
   - Если NOW > reminder_datetime → отправить немедленное уведомление
   - Если NOW <= reminder_datetime → подождать запланированного времени
5. При отправке немедленного уведомления:
   - Отправить клиенту с кнопками подтверждения
   - Установить reminder_24h_sent = true
   - Установить reminder_1_sent_at = NOW
   - Это гарантирует работу цепочки: второе → третье → автоотмена
```

**Формат уведомления:**
```
Придешь на тренировку {дата} в {время}?
[✅ Подтверждаю] [❌ Не смогу прийти]
```

**Гарантии работы цепочки напоминаний:**
- **Второе напоминание:** отправится через X часов после немедленного уведомления
- **Третье напоминание:** отправится через Y часов после второго
- **Автоотмена:** сработает через Z часов после третьего, если клиент не подтвердит

**Файлы реализации:**
- `backend/services/notifications.py` - функция `notify_booking_created_by_trainer()`
- Строки 528-631: логика проверки времени и отправки немедленного уведомления
- Строки 614-624: установка флагов reminder_24h_sent и reminder_1_sent_at

**Примеры сценариев:**

*Сценарий A: Тренировка создана вовремя*
- Настройки: reminder_1_days_before=1, reminder_1_time="12:00"
- Тренировка на 25.11 в 18:00
- Создана: 24.11 в 10:00 (ДО времени напоминания)
- Результат: Немедленное уведомление НЕ отправляется, ждем 24.11 12:00

*Сценарий B: Тренировка создана поздно*
- Настройки: reminder_1_days_before=1, reminder_1_time="12:00"
- Тренировка на 25.11 в 18:00
- Создана: 24.11 в 14:00 (ПОСЛЕ времени напоминания)
- Результат: Немедленное уведомление отправляется сразу в 14:00
- Далее: Второе напоминание в 16:00, третье в 20:00 (по настройкам тренера)
